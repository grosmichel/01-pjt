...
if file_path.exists():  # 파일이 존재할 경우
    ...
    sorted_books = sorted(books, key=lambda x : calculate_discount_rate(x), reverse=True)

    # 5. 상위 5개 도서 선택
    top_discounted = sorted_books[:5]

    # 6. 결과 출력
    print("할인율 상위 5개 도서:")
    for book in top_discounted:
        print(f"{book['title']} - 할인율: {calculate_discount_rate(book)}%")  # 도서 제목과 할인율 출력 (print(f"{book['title']} - 할인율: {discount_rate:.2f}%"))
